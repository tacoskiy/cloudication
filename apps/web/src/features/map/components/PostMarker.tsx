...
const PostMarker = ({ post, onClick }: PostMarkerProps) => {
  ...
  return (
    <Marker
      ...
      anchor="bottom"
    >
      <div ref={elementRef} className="w-16 h-[63px] relative flex items-center justify-center">
        <div
          className={`group flex flex-col items-center cursor-pointer active:scale-95 transition-all duration-300 ${hasAppeared ? "animate-marker-in" : "opacity-0"
            }`}
          onClick={() => onClick(post.id)}
        >
          {/* 雲型マーカー枠 (SVG) */}
          <div className="relative w-16 h-[63px] drop-shadow-2xl transition-transform duration-300 group-hover:scale-120 group-hover:-translate-y-2">
